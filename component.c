...
ice_component_close(component_t *c) {
  ...
  while (!TAILQ_EMPTY(&c->local_candidates)) {
    candidate = TAILQ_FIRST(&c->local_candidates);
    TAILQ_REMOVE(&c->local_candidates, candidate, list);
    candidate_free(candidate);
  }

  while (!TAILQ_EMPTY(&c->remote_candidates)) {
    candidate = TAILQ_FIRST(&c->remote_candidates);
    TAILQ_REMOVE(&c->remote_candidates, candidate, list);
    candidate_free(candidate);
  }
